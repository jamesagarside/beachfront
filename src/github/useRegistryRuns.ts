...
export interface RegistryRunsResult {
  /** Repos that loaded, in Registry order, each with its recent Agent runs. */
  loaded: RepoRuns[];
...
/**
 * Fans the single-repo Agent-runs fetch out across every Registry repo (#10),
 * mirroring {@link useRegistryIssues}: each repo is its own concurrent,
 * individually-cached query, and a repo the Viewer's token can't read fails its
 * own query and is reported as `skipped` rather than breaking the aggregate.
