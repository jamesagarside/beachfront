...
function run(id: number, name: string, status: RunStatus): AgentRun {
  return {
    id,
    name,
    status,
...
describe("RegistryRuns", () => {
  ...
  it("renders recent runs from every accessible repo, grouped by repo, with status and a link", async () => {
    fetchAgentRuns.mockImplementation((_token: string, repo?: RepoRef) =>
      repo?.repo === "two"
        ? Promise.resolve([run(2, "Second repo run", "running")])
        : Promise.resolve([run(1, "First repo run", "succeeded")]),
    );

    renderWithProviders(<RegistryRuns token="t" repos={REPOS} />);

    const alpha = await screen.findByRole("region", { name: /alpha\/one/i });
    const link = within(alpha).getByRole("link", { name: /first repo run/i });
    expect(link).toHaveAttribute(
      "href",
      "https://github.com/o/r/actions/runs/1",
    );
    expect(within(alpha).getByText("succeeded")).toBeInTheDocument();

    const beta = screen.getByRole("region", { name: /beta\/two/i });
    expect(within(beta).getByText("running")).toBeInTheDocument();
  });
  ...
  it("skips an inaccessible repo without breaking the rest of the view", async () => {
    fetchAgentRuns.mockImplementation((_token: string, repo?: RepoRef) =>
      repo?.repo === "two"
        ...
        : Promise.resolve([run(1, "Visible run", "succeeded")]),
    );
  ...
  it("shows a calm empty state for a repo with no recent runs", async () => {
